...
public abstract class CFMetaInterface implements ICFMetaInterface, ILifeCycle, IUnknown, IMetaInterface{
     ...
     */
    private OCM_SingleReceptacle<IAccept> m_PSR_IAccept;
   ...
   /**
	...
	IUnknown pTest;
	for(int index=0; index<m_pcElems; index++){
		if((IUnknown) m_ppComps.get(index)==pComp){
			inCFGraph=true;
			break;
		}
	}
	...
	if(inCFInts==true){
        ...
        if(pIDel==null){
            ...
            for(int i=0; i<m_ppComps.size();i++){
                IUnknown pCompNested = (IUnknown) ((IUnknown) m_ppComps.get(i)).QueryInterface(rintf);
                if(pCompNested!=null){
                    pComp=pCompNested;
                    break;
                }
    ...
    * @param ppRecpMetaInfo a Vector to be filled with interface meta-information.
