...
/**
 * Calculator component supporting arithmetic plug-in components. Includes a
 * parallel receptacle.
 * 
 * @author Paul Grace
 * @version 1.3
...
public class CalculatorParallel extends OpenCOMComponent implements
		ICalculator, IConnections, ILifeCycle, IUnknown, IMetaInterface, ICallback {

	/**
	 * Requires Interface of type IParallel.
	 */
	public OCM_MultiReceptacleParallel<IParallel> m_PSR_IParallel;
	...
	public CalculatorParallel(IUnknown binder) {
		super(binder);
		m_PSR_IParallel = new OCM_MultiReceptacleParallel<IParallel>(
				IParallel.class, this);
	...
	public String display(String message) {
		...
		m_PSR_IParallel.m_pIntf.ParallelExecution(message);
		return returnStr;
	}
	...
	public boolean connect(IUnknown pSinkIntf, String riid, long provConnID) {
		if (riid.toString().equalsIgnoreCase("AdderComponent.IAdd")) {
			return m_PSR_IAdd.connectToRecp(pSinkIntf, riid, provConnID);
				...
				"SubtractComponent.ISubtract")) {
			return m_PSR_ISubtract.connectToRecp(pSinkIntf, riid, provConnID);
		} else if (riid.toString().equalsIgnoreCase("AdderComponent.IParallel")) {
			return m_PSR_IParallel.connectToRecp(pSinkIntf, riid, provConnID);
		}
		return false;
	}
	...
	public boolean disconnect(String riid, long connID) {
		...
		if (riid.toString().equalsIgnoreCase("AdderComponent.IAdd")) {
			...
			return m_PSR_IParallel.disconnectFromRecp(connID);
		}
		return false;
	}
	...
	public void sendResult(CallbackResult result) {
		System.out.println("[" + this + "] received " + result.getValue() + " from " + result.getFrom());
	}
