...
public class CalculatorSequential extends OpenCOMComponent implements ICalculator, IConnections, ILifeCycle, IUnknown, IMetaInterface {
     ...
     */
    public OCM_MultiReceptacle<IParallel> m_PSR_IParallel;
    ...
    public CalculatorSequential(IUnknown binder) {
        super(binder);
	m_PSR_IParallel = new OCM_MultiReceptacle<IParallel>(IParallel.class);
	...
	for(int i=0; i<m_PSR_IParallel.interfaceList.size(); i++){
		m_PSR_IParallel.interfaceList.get(i).ParallelExecution(message);
	}
       return returnStr;
    }
